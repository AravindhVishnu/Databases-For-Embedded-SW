...
{
  if (this->initData(UserParameters::ID_USER_PARAMETER_ONE, DataValue::TYPE_INT32, 1, 1, 4, 1, 0, 100, 1, UserParameters::mS, UserParameters::ALL_APPLICATIONS, 0, false) != UserParameters::WRITE_SUCCESS)
  {
    assert(0);
    return false;
  }
  if (this->initData(UserParameters::ID_USER_PARAMETER_TWO, DataValue::TYPE_UINT32, 1000U, 1, 5, 1000U, 0U, 100000U, 2U, UserParameters::kN, UserParameters::ALL_APPLICATIONS, 0, false) != UserParameters::WRITE_SUCCESS)
  {
    assert(0);
    return false;
  }
  if (this->initData(UserParameters::ID_USER_PARAMETER_THREE, DataValue::TYPE_FLOAT, 0.0f, 2, 6, 0.0f, 0.0f, 15.0f, 0.1f, UserParameters::V, UserParameters::ALL_APPLICATIONS, 1, false) != UserParameters::WRITE_SUCCESS)
  {
    assert(0);
    return false;
  }
