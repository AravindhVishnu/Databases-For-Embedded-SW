...
MyExampleClass::MyExampleClass()
{
  ...
  if (this->m_userParametersPtr->init() == false)
  {
    std::cout << "User parameter database initialization failed" << std::endl;
    assert(0);
  }
  this->m_runtimeDataPtr = RuntimeDatabase::instance();
  if (this->m_runtimeDataPtr->init() == false)
  {
    std::cout << "Runtime database initialization failed" << std::endl;
    assert(0);
  }
}
...
void MyExampleClass::exec()
{
  ...
  {
    std::cout << "Writing default values" << std::endl;
  ...
  std::cout << "Step size increment" << std::endl;
  this->m_userParametersPtr->stepSizeIncrement<int32_t>(UserParameters::ID_USER_PARAMETER_ONE);
  this->m_userParametersPtr->stepSizeIncrement<uint32_t>(UserParameters::ID_USER_PARAMETER_TWO);
  this->m_userParametersPtr->stepSizeIncrement<float>(UserParameters::ID_USER_PARAMETER_THREE);

  // Read from database using key
  userParameterOne = this->m_userParametersPtr->getValue<int32_t>(UserParameters::ID_USER_PARAMETER_ONE);
  std::cout << "Reading userParameterOne value " << userParameterOne << " from database" << std::endl;

  userParameterTwo = this->m_userParametersPtr->getValue<uint32_t>(UserParameters::ID_USER_PARAMETER_TWO);
  std::cout << "Reading userParameterTwo value " << userParameterTwo << " from database" << std::endl;

  userParameterThree = this->m_userParametersPtr->getValue<float>(UserParameters::ID_USER_PARAMETER_THREE);
  std::cout << "Reading userParameterThree value " << userParameterThree << " from database" << std::endl;

  std::cout << std::endl;

  std::cout << "Step size decrement" << std::endl;
  this->m_userParametersPtr->stepSizeDecrement<int32_t>(UserParameters::ID_USER_PARAMETER_ONE);
  this->m_userParametersPtr->stepSizeDecrement<uint32_t>(UserParameters::ID_USER_PARAMETER_TWO);
  this->m_userParametersPtr->stepSizeDecrement<float>(UserParameters::ID_USER_PARAMETER_THREE);
